...
class URLRuleBuilder {
    private val predicates: MutableList<(urls: List<Url>) -> List<List<Line>>> = mutableListOf()
    private var type: RuleViolationType = RuleViolationType.Error
    private var name: String = "Rule name"
    private var region: PDFRegion = PDFRegion.EVERYWHERE

    fun disallow(predicate: (urls: List<Url>) -> List<List<Line>>) = this.also { predicates.add(predicate) }
