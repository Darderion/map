...
export default class PDFTextComponent extends Vue {
	...
	updatePDFView() {
		if (this.receivedLines &&
			...
			!!document.getElementById(`pdfLine${this.pdfLines[this.pdfLines.length - 1].line}-${this.pdfLines[this.pdfLines.length - 1].page}`)) {
			this.pdfLines.forEach(it => {
				if (!document.getElementById(`pdfLine${it.line}-${it.page}`)) {
				}
				document.getElementById(`pdfLine${it.line}-${it.page}`)!.style.backgroundColor = '#abc'
			...
			if (this.viewMode == ViewMode.Areas) {
				this.pdfLines.forEach(it => {
					document.getElementById(`pdfLine${it.line}-${it.page}`)!.style.color = '#abc'
